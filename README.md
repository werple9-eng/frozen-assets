...
- Move the pointer to aim. Click with the starter chisel. New files begin with Tony's two-parcel induction: recover two coins, buy **Hold to Chip** with the proceeds, then practice holding around a coin and cash bundle. Later tools strike, break or melt.
...
- Open **Upgrades** (**U**) for six separate radial tool trees and 70 authored upgrades. Choose an unlocked tool across the top; undiscovered tools show **?**. Hover or focus a node for its effect and price. Drag to pan, use the wheel or right-hand rail to zoom, and use arrow keys for nearby-node navigation. Purchases use your net balance after Tony's commission. Equipment arrives through maintenance requisition cards; each later tool has a separate paid acquisition. The bench pauses while menus are open.
- Click the landline or press **P** to pick up. Campaign calls ring three times, then wait silently with a steady light while work continues. Live calls pause the work and wait for **Next**; one confirmation reveals unfinished text silently, the next advances. Pick up an idle handset and dial **7** for any queued call; **Esc** hangs up an outgoing line. Click the manila folder or press **F** for **Recovery Files**, the separate call/evidence archive. **[ / ]** cycles owned tools.
...
- Press **Esc** or the F/A mark to pause. Restart and the control guide live here. The separate **Settings** screen (**O**) contains audio, dialogue type sounds, text speed, tray sensitivity, Low/Medium/High/Ultra graphics, fewer particles, reduced motion, and larger text.
- Finish 32 main deliveries across Small Change, Cold Storage, The Audit, Sublevel B and The Vault. Claims contain varied inner compartments, progressing from small parcels to seams, supported wings, and archives. The five-part final vault contains the Freeze Ledger. Answer Tony's quiet postgame coda to unlock Recovery Contracts.

Saves are automatic and local to this browser and origin. Version 5 compresses active ice and retains campaign state, commissions, evidence, message history, tools and compartments. Earlier saves preserve earned funds and paid reward identities. Very old layouts that cannot fit the current field regenerate only their active parcel, with a migration diagnostic. The original `frozen-assets-v3` browser key is imported into file 1 when no catalog exists and remains untouched. The three-file catalog uses `frozen-assets-slots-v1`. Restart clears progress but retains comfort settings. The isolated `?qa=1` bench is disposable and does not write the regular save.
...
The narrative/economy revision 2 pass keeps seven Tony calls (including the coda) and three Mercer calls outside the unchanged tutorial. Routine tool, chapter and delivery phone barks are retired. Ordinary cargo uses fixed catalog bases: loose coin $15, cash $55, small gold bar $150, with later physically distinct watches, chains, bonds, gem cases and bullion. Existing wallets and paid rewards are preserved; unearned legacy cargo adopts reduced values without moving current layout-3 cargo. See **QA-NARRATIVE-ECONOMY.md** for validation and remaining pacing failures. The reference-driven Upgrades redesign remains pending the missing original video; **QA-REFERENCE-TREE-VISUAL.md** records the preparatory static study.
...
The two induction parcels use the same physical-contact release rule as the campaign: a find releases when no live ice contacts its mesh. The first $30 gross claim deducts Tony's $3 fee; **Hold to Chip** costs $25 from the remaining $27. The second parcel contains a $15 coin and $55 cash bundle. Both parcels together leave $64 after commission and the fitting purchase. Old active tutorial parcels retain their saved geometry. A stamped assignment board introduces all five chapters, followed by a final live call and the Chapter 1 title.
...
Tutorial revision 2 saves the dialogue/task/ringing state and preserves prior parcel layouts when loading older checkpoints. Active campaign calls save the current panel and ring/pending state. The preview remains local; GitHub source synchronization is separate from publishing a hosted game.
